 ...
 * {@link ConceptTreeTransformProvider}.
 * <p>Note: the tasks are not run;
 * this must be done afterwards using the API, by a suitable user.
 * </p>
 * <p>Here is a suggested way to use this program.</p>
 * <pre> ant migrate-jsontree-artefacts &gt; migrate-out.txt
 * grep 'You must now' migrate-out.txt &gt; migrate-ids.txt</pre>
 * Have a look in migrate-ids.txt; you'll see the task Ids of the tasks
 * that need to be run. If you're "lucky" (i.e., no one else was publishing
 * at the same time), this is a contiguous sequence of integers.
 * Let's say the numbers go from 378 to 462.
 * As your normal user, define a suitable user authentication cookie
 * for the Registry as an environment variable,
 * and run the tasks:
 * <pre> COOKIE=a%3A11%3A%7Bs%3A10%3A%22session_id%22..........
 * for i in $(seq 378 462) ; do
 *   echo $i &amp;&gt;&gt; curl-out.txt
 *   echo curl -X PUT --header 'Content-Type: application/json' \
 *     --header 'Accept: application/xml' \
 *     --header "ands_authentication: $COOKIE" \
 *     "http://localhost:8080/vocabs-registry/adminApi/workflow/tasks/$i" \
 *     &amp;&gt;&gt; curl-out.txt
 ...
 *     &amp;&gt;&gt; curl-out.txt
 * done</pre>
 * Have a look in {@code curl-out.txt} to confirm it worked OK.
 */
